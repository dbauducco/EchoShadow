...
; Updated for newer AutoIt3 compiler, confirmed with v3.3.14.5
; #pragma compile(UPX, False) ; Doesn't seem to work...
#pragma compile(x64, False)
#pragma compile(FileDescription, WinSendKeys)
#pragma compile(FileVersion, 3.2.2.2)
#pragma compile(LegalCopyright, (c) 2011..2020 Ath)
#pragma compile(Icon, AU3_Icon_1.ico) ; Is no longer added by default...

#AutoIt3Wrapper_UseUpx=n
; #AutoIt3Wrapper_UseX64=n
; #AutoIt3Wrapper_Res_Description=WinSendKeys
; #AutoIt3Wrapper_Res_Fileversion=3.2.2.2
; #AutoIt3Wrapper_Res_LegalCopyright=(c) 2011..2020 Ath
; #AutoIt3Wrapper_Res_Language=1033
; #AutoIt3Wrapper_Icon=AU3_Icon_1.ico
...
; History:
; 2020-05-06, version 3.2.2.2
; ! Fixed that when using -f <strokesfile> also a <windowname> argument was required, as the windowname can also be specified in the strokesfile.
; 2020-05-05, version 3.2.2.1
; ! Fixed strange error of undefined constants in AutoIt standard library sources, ven though these are defined. Possibly a compiler issue.
; 2018-11-04, version 3.2.2.0
; + Added -i <msec> option and initialdelay ini setting for an initial delay before any keystrokes are sent
; + Added -s <msec> option and senddelay ini setting for overriding the default 5 msec delay between individual characters sent
; + Added -sd <msec> option and senddowndelay ini setting for overriding the default 5 msec time a key is held down for each individual character sent
; 2018-11-03, version 3.2.1.1
; + Added setting exit code to 1 if the window is not found
...
Global $initdela = 0 ; Initial delay before sending any keystrokes, default 0
Global $win = "" ; Name of Window to activate
Global $dbg = 0 ; Debug mode
Global $senddelay=5 ; msec delay between each character sent by the Send() function
Global $senddowndelay = 5 ; msec time to hold down any key 'pressed' during the Send() function
...
$help = 0 ; Display help?
$exiterror = 0 ; Default errorlevel
#endregion
...
#region Commandline parameters
	...
	While $n <= $CmdLine[0]
		If ($p = $n) And (StringLeft($CmdLine[$n], 1) = "-") Then
			...
			EndIf
			If (StringLower($CmdLine[$n]) = "-i") And ($n < $CmdLine[0]) Then
				$n += 1
				$initdela = $CmdLine[$n]
				$p = $n + 1
				If $dbg Then writeLogLine("Initial send delay (-i): " & $initdela)
			EndIf
			If (StringLower($CmdLine[$n]) = "-s") And ($n < $CmdLine[0]) Then
				$n += 1
				$senddelay = $CmdLine[$n]
				$p = $n + 1
				If $dbg Then writeLogLine("Send delay (-s): " & $senddelay)
			EndIf
			If (StringLower($CmdLine[$n]) = "-sd") And ($n < $CmdLine[0]) Then
				$n += 1
				$senddowndelay = $CmdLine[$n]
				$p = $n + 1
				If $dbg Then writeLogLine("Send down-delay (-sd): " & $senddowndelay)
			EndIf
	...
	If ($win = "" And $xfile = "" And $fname = "") Or $help > 0 Then
		ShowHelp()
	Else
		If $senddelay <> 5 Then
			AutoItSetOption("SendKeyDelay", $senddelay)
			If $dbg Then writeLogLine("Send delay (-s) of " & $senddelay & " msec set.")
		EndIf
		If $senddowndelay <> 5 Then
			AutoItSetOption("SendKeyDownDelay", $senddowndelay)
			If $dbg Then writeLogLine("Send down delay (-sd) of " & $senddowndelay & " msec set.")
		EndIf
		...
		Else
			...
			If $initdela > 0 Then
				Sleep($initdela)
				If $dbg Then writeLogLine("Initial delay of " & $initdela & " msec waited.")
			EndIf
...
#region Clipboard handling
	...
	EndIf
EndIf
If $dbg Then writeLogLine("Exiting application with errorlevel '" & $exiterror & "'.")
Exit ($exiterror)
#endregion
...
Func handleCommand($cmd)
	...
	If Not $handled Then
		If $varexp Then ; Explode variables
			...
			$cmd = $tmp
		EndIf
		If $dbg Then writeLogLine("Send: " & $cmd)
		Send($cmd)
	EndIf
...
Func ShowHelp()
	MsgBox(0, "WinSendKeys " & $appVersion & " : Send keystrokes and/or mouse-clicks to a window", _
			"WinSendKeys [-t delay] [-d] [-m|-mm speed] [-f strokefile] [-c] [-cc] [-cn]" & @CRLF _
			 ...
			 & @TAB & "[-i delay] [-s delay] [-sd delay]" & @CRLF _
			 & @TAB & "[-?] [-w] <WindowName> [<KeyStrokes/MouseStrokes>] [...]" & @CRLF _
			 & "Options:" & @CRLF _
			 & "-t delay = msec value, default 100" & @CRLF _
			 & "-i delay = msec value, initial delay, default 0" & @CRLF _
			 & "-s delay = msec value, between-character delay, default 5" & @CRLF _
			 & "-sd delay = msec value, keydown time per character, default 5" & @CRLF _
...
Func readIniFile()
	...
	If $ini <> "" And FileExists($ini) Then
		...
		; $initdela = 0 ; msec delay before any keystrokes sent
		$initdela = IniRead($ini, "Settings", "initaldelay", $initdela)
		If $dbg And $initdela <> 0 Then writeLogLine("Initial delay from ini: " & $initdela & @CRLF)

		; $senddelay = 5 ; msec delay between any character sent in Send() function
		$senddelay = IniRead($ini, "Settings", "senddelay", $initdela)
		If $dbg And $senddelay <> 5 Then writeLogLine("Send delay from ini: " & $senddelay & @CRLF)

		; $senddowndelay = 5 ; msec key hold down time for each character/key sent in Send() function
		$senddowndelay = IniRead($ini, "Settings", "senddowndelay", $initdela)
		If $dbg And $senddowndelay <> 5 Then writeLogLine("Send down delay from ini: " & $senddowndelay & @CRLF)
